...
## [1.8.0-zh] - 2026-07-06 · 文件管家中文定制版

文件管家中文定制版更新（下方为继承的历史版本记录）：

- 全新左侧导航栏布局（文件整理 / 文件清理双页面），替代原单页界面
- 强制白色浅色主题，Mac / Windows 观感一致，不随系统深色模式变化
- 全新蓝色渐变应用图标
- **新增「文件清理」功能**：垃圾缓存 / 重复文件 / 大文件 / 空项目四类只读扫描，
  只统计定位、绝不删除，一键跳转文件管理器
- 界面语言与 AI 分类输出语言合并为一个选择（15 种语言），默认简体中文
- 分类粒度优化：默认只建一级分类文件夹，提示词强制归并到常见大类
- 文件浏览器侧栏默认隐藏（视图菜单可开）
- 完整简体中文本地化（含全部新增界面文案）
- README 多语言化（中文默认 + 14 种翻译）
